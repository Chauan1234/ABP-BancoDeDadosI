...
export default function AtualizarProdutoPage() {
    ...
    useEffect(() => {
        ...
        async function load() {
            ...
            try {
                ...
                const res = await fetch(`${baseUrl}/produtos/${codigo}`);
                if (!res.ok) throw new Error(`Erro ${res.status}`);
                const data = await res.json();
                if (mounted) setForm({ nome: data.nome ?? '', preco: data.preco ? String(data.preco) : '', descricao: data.descricao ?? '', tamanho: data.tamanho ? String(data.tamanho) : '', quantidade: data.quantidade ? String(data.quantidade) : '', codfabricacao: data.codfabricacao ? String(data.codfabricacao) : '' });
            } catch (err: unknown) { toast.error(getErrorMessage(err) ?? 'Erro ao carregar', { closeButton: true }); }
    ...
    useEffect(() => {
        let mounted = true;
        async function loadFabricacoes() {
            try {
                const baseUrl = process.env.NEXT_PUBLIC_API_URL ?? 'http://localhost:3001';
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        try {
            const baseUrl = process.env.NEXT_PUBLIC_API_URL ?? 'http://localhost:3001';
            const res = await fetch(`${baseUrl}/produtos/${codigo}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ nome: form.nome, preco: Number(form.preco || 0), descricao: form.descricao, tamanho: form.tamanho ? Number(form.tamanho) : null, quantidade: form.quantidade ? Number(form.quantidade) : null, codfabricacao: form.codfabricacao && form.codfabricacao !== 'none' ? Number(form.codfabricacao) : null }) });
            if (!res.ok) { const json = await res.json().catch(() => null); throw new Error(json?.error ?? `Status ${res.status}`); }
            toast.success('Produto atualizado com sucesso', { closeButton: true }); router.push('/produtos');
    ...
    return (
        <div className="max-w-3xl mx-auto p-6">
            <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                    <h1 className="text-2xl font-semibold">Atualizar Produto</h1>
                    <p className="text-sm text-muted-foreground">Atualize os detalhes do produto no seu sistema</p>
                </div>
            </div>
