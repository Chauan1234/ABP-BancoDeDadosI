...
export default function DeletarFabricacaoModal({ open, onOpenChange, codigo, onDeleted }: Props) {
    ...
    async function handleExcluir() {
        ...
        try {
            ...
            const res = await fetch(`${baseUrl}/fabricacoes/${codigo}`, { method: 'DELETE' });
            const json = await res.json().catch(() => null);
            if (!res.ok) { toast.error(json?.error ?? `Status ${res.status}`, { closeButton: true }); return; }
            toast.success(json?.message ?? 'Fabricação removida com sucesso', { closeButton: true });
        ...
        <AlertDialog open={open} onOpenChange={onOpenChange}>
            <AlertDialogContent>
                <AlertDialogHeader>
                    ...
                    <AlertDialogTitle className="text-center">Tem certeza que deseja excluir esta fabricação?</AlertDialogTitle>
                    <AlertDialogDescription className="text-center">Esta ação não pode ser desfeita. Isso excluirá permanentemente a fabricação.</AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
